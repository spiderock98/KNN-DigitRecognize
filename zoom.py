...
class Zoom:
    def __init__(self, ZoomSize, src, y0, y1, x0, x1):
        ...
        self.dim = (self.sizeofImage, int(
            self.h * (self.sizeofImage / float(self.w))))
    ...
    def update(self, e):
        while e.wait():
            ...
            self.gammaAdj = gamma(self.zoom, cv2.getTrackbarPos(
                'Gamma', 'Panel{}'.format(self.src)))
            self.barGauss = cv2.getTrackbarPos(
                'Gaussian', 'Panel{}'.format(self.src))
            self.gauss = cv2.GaussianBlur(
                self.gammaAdj, (self.barGauss * 2 + 1, self.barGauss * 2 + 1), 0)
            ...
            self.dst = cv2.dilate(self.thresh, None, iterations=cv2.getTrackbarPos(
                'Dilate', 'Panel{}'.format(self.src)))
